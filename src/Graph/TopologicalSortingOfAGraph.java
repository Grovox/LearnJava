...
public class TopologicalSortingOfAGraph {
    static void DFS(Node node, HashSet<Node> passed, LinkedList<Node> order){
        passed.add(node);
        for (Edge edge : node.edges){
            if (!passed.contains(edge.adjacentNode)){
                DFS(edge.adjacentNode,passed,order);
            }
        }
        order.addLast(node);
    }

    static void DFSWrap(HashMap<Integer,Node> graph, LinkedList<Node> order){
    ...
    public static void main(String[] args) {
        ...
        LinkedList<Node> sortedOrder = new LinkedList<>();
        DFSWrap(graph, sortedOrder);

        System.out.println(sortedOrder.stream().map(s -> s.value).collect(Collectors.toList()));
    }
}
